...
ClearPlayField
			PUSH	{R0-R5,LR}
			MOV		R0, #6				;setting x and y to the pixel in playfield
			MOV		R1, #4
clrNxtRow		
			LDR		R2,=GPIO_PORTA_DATA		; set PA6 low for Command
			LDR		R3,[R2]
			BIC		R3,#0x40
			STR		R3,[R2]
			MOV		R5,#0x20				; ensure H=0
			...
			BL		Out1BNokia	
ClrReady		
			LDR		R2,=SSI0_SR				; wait until SSI is done
			LDR		R3,[R2]
			ANDS	R3,R3,#0x10
			BNE		ClrReady
			LDR		R2,=GPIO_PORTA_DATA		; set PA6 high for Data
			LDR		R3,[R2]
			ORR		R3,#0x40
			STR		R3,[R2]	
			MOV		R2,#64					
